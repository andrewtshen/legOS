...
void MPU_setup() {
    ...
    MPU->RBAR = 0x00000011; // Base Address = 0x00000000
    MPU->RASR = 0x0307003F; // Privileged only, TEX=0,S=0,C=0,B=0, 128MB, Enable=1
    //                         // ---
    //                         // Disable Kernel RAM
    // MPU->RNR = 1;           // Select region 1
    MPU->RBAR = 0x20000012; // Base Address = 0x20000000
    MPU->RASR = 0x0307001B; // Privileged only, TEX=0,S=0,C=0,B=0, 16KB, Enable=1
    //                         // ---
    //                         // Enable User Prog
    // MPU->RNR = 2;           // Select region 2
    MPU->RBAR = 0x20005013; // Base Address = 0x20005000
    MPU->RASR = 0x0107001B; // Any access, TEX=0,S=0,C=0,B=0, 16KB, Enable=1
